...
struct MaxLengthRuleTests {
    ...
    func valid(target: String, inputLength: Int) throws {
        #expect(target.cv.maxLength(inputLength).validate() == true)
    }

    @Test(arguments: zip(["test", "abc", "", "12345", "あいう"], [5, 4, 10, 6, 4]))
    func
    validLessThanArgment(target: String, inputLength: Int) throws {
        #expect(target.cv.maxLength(inputLength).validate() == true)
    }
    ...
    invalidMoreThanArgment(target: String, inputLength: Int) throws {
        #expect(target.cv.maxLength(inputLength).validate() == false)
    }
}
